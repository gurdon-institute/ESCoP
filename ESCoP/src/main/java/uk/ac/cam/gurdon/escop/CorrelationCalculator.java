...
public class CorrelationCalculator implements Runnable{
	...
	public CorrelationCalculator(Data3D A, Data3D B, Roi roi, double thresholdA, double thresholdB, int shift, Axis axis, AtomicInteger count) throws IllegalArgumentException{	//CCF
		if(A.size()!=B.size()){
	...
	public CorrelationCalculator(Data3D A, Data3D B, Roi roi, double thresholdA, double thresholdB) throws IllegalArgumentException{	//PCC without shift
		if(A.size()!=B.size()){
			throw new IllegalArgumentException("Data3D are different sizes.");
		}
		this.A = A;
